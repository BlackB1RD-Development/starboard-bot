...
class StarBoardClient extends Client {
  ...
  handleMessage(message) {
    ...
    if (cmd) {
      try {
        ...
        this.emit('cmdError', this, message, cmd, error);
      } finally {
  ...
  _registerCommand(command) {
    ...
    else if (!this.categories.get(command.category)) this.categories.set(command.categories, new Category(command.category));
